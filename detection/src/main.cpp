...
int main()
{
	...
	try
	{
		cv::Mat image = cv::imread(std::string(BINARY_FILE_PATH) + std::string("../images/2007_005331_pred.jpg"));
		Detector detector;
//	detector.Initialize(-1, 416, 416, std::string(BINARY_FILE_PATH) + std::string("../name.txt"));
//#ifdef TRAIN
//	detector.Train("D:\\AllentFiles\\data\\dataset4work\\detection", ".jpg", 30, 4, 0.001, "weights/detector.pt", "weights/yolo4_tiny.pt");
//#elif TEST
