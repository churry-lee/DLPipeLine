...
int main()
{
	...
	cv::Mat image = cv::imread(std::string(PROJECT_SRC_DIR) + std::string("/images/2007_005331.jpg"));
	Detector detector;
	detector.Initialize(-1, 416, 416, std::string(PROJECT_SRC_DIR) + std::string("name.txt"));
#ifdef TRAIN
