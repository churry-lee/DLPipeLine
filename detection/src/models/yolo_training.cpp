...
YOLOLossImpl::YOLOLossImpl(torch::Tensor anchors_, int num_classes_, int img_size_[], float label_smooth_, torch::Device device_, bool normalize)
{
...
std::vector<int> nms_libtorch(torch::Tensor bboxes, torch::Tensor scores, float thresh)
{
	...
	while (order.numel() > 0)
	{
		...
		if (order.numel() == 1)
		{
			...
			auto i = order[0].item();// scores box[i]
			keep.push_back(i.toInt());
		}
		//box[i] IOU()
		...
		auto idx = (iou <= thresh).nonzero().squeeze();//idx[N - 1, ] order[N, ]
		if (idx.numel() == 0)
			break;
